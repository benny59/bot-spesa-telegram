...
SERVICE_DIR="${SERVICE_DIR:-/data/data/com.termux/files/usr/var/service}"
...
list_runsvdir_pids() {
  pgrep -f "[r]unsvdir $SERVICE_DIR" 2>/dev/null
}
...
ensure_services_up() {
  if [ -x "$SV_BIN" ]; then
    "$SV_BIN" up sshd >> "$LOG_FILE" 2>&1 || true
    "$SV_BIN" up crond >> "$LOG_FILE" 2>&1 || true
  else
...
main() {
  ...
  log "WINDOW_SECONDS=$WINDOW_SECONDS SLEEP_SECONDS=$SLEEP_SECONDS SERVICE_DIR=$SERVICE_DIR"
