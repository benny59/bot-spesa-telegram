...
Dir.mktmpdir("config-preferiti-visibility-test") do |directory|
  Dir.chdir(directory) do
    ...
    DB.execute(
      "INSERT INTO items (gruppo_id, topic_id, creato_da, nome) VALUES (0, 0, ?, ?)",
      [user_id, DataManager::CONFIG_PREFERITI_PREFISSO]
    )
    config_legacy_id = DB.last_insert_row_id
    DB.execute(
      "INSERT INTO items (gruppo_id, topic_id, creato_da, nome) VALUES (0, 0, ?, ?)",
      [user_id, DataManager::CONFIG_PREFERITI_NOME]
    )

    nomi_telegram = DataManager.prendi_articoli_ordinati(0, 0).map { |item| item["nome"] }
    raise "La configurazione preferiti legacy e' visibile su Telegram" if nomi_telegram.include?(DataManager::CONFIG_PREFERITI_PREFISSO)
    raise "La configurazione preferiti corrente e' visibile su Telegram" if nomi_telegram.include?(DataManager::CONFIG_PREFERITI_NOME)
    ...
    configurazioni_salvate = DB.get_first_value(
      ...
      ["#{DataManager::CONFIG_PREFERITI_PREFISSO}%"]
    ).to_i
