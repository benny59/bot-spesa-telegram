    ...
    fun getProductPreview(barcode: String, forceRefresh: Boolean = false): ProductPreview? {
        ...
        return http.newCall(req).execute().use { response ->
            ...
            ProductPreview(
                ...
                manufacturingPlaces = raw["manufacturing_places"] as? String ?: "",
                nutriscoreGrade = raw["nutriscore_grade"] as? String ?: "",
                novaGroup = (raw["nova_group"] as? Double)?.toInt(),
                additives = (raw["additives"] as? List<*>)?.filterIsInstance<String>().orEmpty(),
                allergens = (raw["allergens"] as? List<*>)?.filterIsInstance<String>().orEmpty(),
                ingredientsText = raw["ingredients_text"] as? String ?: "",
                completeness = raw["completeness"] as? Double,
    ...
    private fun parseItems(body: String): List<SpesaItem> {
        ...
        return raw.map { item ->
            ...
            SpesaItem(
                ...
                telegramPhotoId = item["picture_id"] as? String,
                telegramPhotoFileName = item["picture_file_name"] as? String,
                telegramPhotoDate = item["picture_date"] as? String,
