  ...
  def self.genera_barcode_con_nome(codice, nome, user_id)
    ...
    begin
      ...
      when :ean13
        barcode = Barby::EAN13.new(codice[0..11])
      when :itf14, :itf
        # Per ITF, usa Code128 come fallback (Barby non supporta ITF direttamente)
      ...
      # SOLO PER CODICI PROBLEMATICI, PROVA TEC-IT
      codici_problematici = ["400000091952"]  # Lista codici che danno problemi con Barby
      
      if codici_problematici.include?(codice)
        puts "🔄 [TEC-IT] Codice problematico, provo con servizio esterno"
        begin
          ...
          if response.status == 200 && response.body.size > 1000
            File.open(img_path, 'wb') { |f| f.write(response.body) }
            puts "✅ [TEC-IT] Barcode problematico risolto: #{img_path}"
            return { success: true, img_path: img_path, formato: formato, provider: :tec_it }
          else
            raise "TEC-IT non ha restituito un'immagine valida"
          end
        rescue => e2
          puts "❌ [TEC-IT] Fallito: #{e2.message}"
        end
      end
      
      # ULTIMO FALLBACK: CODE128
      puts "🔄 [FALLBACK] Uso Code128"
      barcode = Barby::Code128.new(codice)
      png_data = barcode.to_png(height: 250, margin: 25, xdim: 4)
      File.open(img_path, 'wb') { |f| f.write(png_data) }
